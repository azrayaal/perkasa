...
<template>
  ...
  <!--
    ...
    konten putih — jadi seluruh warnanya memakai token `sidebar-*`.
  -->
  <aside
  ...
  >
    ...
    >
      <!--
        Logo berupa JPEG hijau di atas latar PUTIH (tanpa transparansi). Kalau
        ditempel langsung ke sidebar gelap ia jadi kotak putih; jadi dibungkus
        ubin putih membulat supaya terlihat disengaja dan warna brand utuh.
      -->
      <img
        src="/ginkgo_living_logo.jpeg"
        alt="Ginkgo Living"
        class="h-10 w-10 shrink-0 rounded-rail bg-white object-contain p-0.5"
      />
      <div class="leading-tight" :class="labelClass">
        <p class="font-semibold text-sidebar-ink-strong">Ginkgo Living</p>
        <p class="text-xs text-sidebar-ink">Resident Journey ERP</p>
      </div>
    </div>
    ...
    <footer v-if="auth.user" class="px-3 pb-4">
      <div
