 ...
 *
 * Semua service memanggil `respond()` alih-alih mengembalikan array mock
 * langsung, supaya kontraknya (async + latency + clone) identik dengan HTTP call.
 *
 * TODO: replace with real API call — ganti isi `respond()` dengan `fetch()`
 * ke API gateway; signature service di atasnya tidak perlu berubah sama sekali.
 */
...
export function respond<T>(payload: T): Promise<T> {
  // structuredClone mencegah komponen tidak sengaja memutasi mock data global —
